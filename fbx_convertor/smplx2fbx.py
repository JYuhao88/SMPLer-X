...
class FbxConvertor:
    def __init__(self, smplx_model_fbx_path, blend_type:list=["betas", "expressions"], fbx_file_format=0):
    ...
    def smplxmotion2fbx(self, smplx_motions, save_path, fps=24):
    ...
    def set_rotation_curve(
        ...
        for joint_idx, joint in enumerate(SMPLX_JOINTS.orig_joints_name):
            ...
            for chn_idx, chn in enumerate("XYZ"):
                lAnimCurve = pNode.LclRotation.GetCurve(pAnimLayer, chn, True)
                lTime = fbx.FbxTime()
                lAnimCurve.KeyModifyBegin()
